...
/*	
*	---------------------------------------------------------------------
*	TheWhiteCollars Demo data importer
*	--------------------------------------------------------------------- 
*/
...
add_action( 'admin_init', 'collars_importer' );
function collars_importer() {
    global $wpdb;
    ...
    if ( current_user_can( 'manage_options' ) && isset( $_GET['import_data_content'] ) ) {
        ...
        if ( ! class_exists('WP_Import') ) {
            $wp_import = COLLARS_PLUGINS . '/importer/wordpress-importer.php';
            require $wp_import;
        }
        ...
        if ( class_exists( 'WP_Importer' ) && class_exists( 'WP_Import' ) ) {
			...
			// Import demo data
            $theme_xml = COLLARS_PLUGINS . '/importer/data/theme_data.xml';
            ...
            $widget_data = COLLARS_PLUGINS . '/importer/data/widget_data.wie';
			collars_process_import_file( $widget_data );
...
function collars_process_import_file( $widget_data ) {
	...
	collars_import_data( $data );

}

// Available widgets
function collars_available_widgets() {
	...
	return apply_filters( 'collars_available_widgets', $available_widgets );
}

// Import widget JSON data
function collars_import_data( $data ) {
	...
	$available_widgets = collars_available_widgets();
